...
void FloorPlanner::evaluateInterconnects(FloorPlanner::Cost& cost, double const& frequency, std::vector<CorblivarAlignmentReq> const& alignments, bool const& set_max_cost, bool const& finalize) {
			...
			for (i = cur_net.layer_bottom; i <= cur_net.layer_top; i++) {
				...
				// clustering itself
					...
					if (i < cur_net.layer_top) {
						...
						//
						if (finalize) {
							TSV_Island::greedyShifting(this->TSVs.back(), this->TSVs, this->blocks, this->IC.outline_x, this->IC.outline_y, (this->techParameters.bond_thickness == 0.0));
						}
					}
				}
			}
	...
	if (this->layoutOp.parameters.signal_TSV_clustering && !this->layoutOp.parameters.trivial_HPWL) {
		...
		// actual clustering
		this->clustering.clusterSignalTSVs(this->nets, nets_segments, this->TSVs, this->blocks, this->IC.outline_x, this->IC.outline_y, this->techParameters.TSV_pitch, this->techParameters.TSV_per_cluster_limit, this->thermal_analysis, (this->techParameters.bond_thickness == 0.0));
		...
		if (derive_TSVs || finalize) {
			...
			if (intersect.area != 0.0) {
				...
				for (layer = min_layer; layer < max_layer; layer++) {
					...
					//
					TSV_Island::greedyShifting(island, this->TSVs, this->blocks, this->IC.outline_x, this->IC.outline_y, (this->techParameters.bond_thickness == 0.0));
